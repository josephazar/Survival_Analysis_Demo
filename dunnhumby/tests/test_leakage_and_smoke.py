...
1. Landmark feature matrix contains no post-landmark data.
...
from config import (CHURN_WINDOW, LANDMARK_DAY, MAX_DAY, PROCESSED_DIR, REPO_ROOT)
...
def test_landmark_features_are_pre_t0():
...
def test_no_leakage_correlations():
...
def test_survival_probs_monotone():
    print("\n3. Scorecard S(30d) >= S(60d) >= S(90d) >= S(180d)")
    scorecard = pd.read_csv(REPO_ROOT / "data" / "household_survival_scorecard.csv")
...
def test_scorecard_shape_and_nulls():
    ...
    # CLV bench columns are expected null for non-test households
...
def test_event_time_consistency():
    ...
    expected = ev["last_overall"] + CHURN_WINDOW - LANDMARK_DAY
    total += 1
    passed += assert_true(
        (ev["event_time"] == expected).all(),
        f"event=1 HHs have event_time = last_overall + {CHURN_WINDOW} - {LANDMARK_DAY}"
    )
...
def main():
    ...
    print("=" * 64)
    results = []
    for fn in [test_landmark_features_are_pre_t0,
                test_no_leakage_correlations,
                test_survival_probs_monotone,
                test_scorecard_shape_and_nulls,
                test_clv_split_disjoint,
                test_event_time_consistency]:
        results.append(fn())
